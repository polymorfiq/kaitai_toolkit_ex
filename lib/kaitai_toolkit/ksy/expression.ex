defmodule KaitaiToolkit.Ksy.Expression do
  ...
  def parse(lexed), do: lexed |> parse_object_refs() |> parse_modifiers() |> parse_bitwise() |> parse_pemdas() |> parse_comparisons() |> parse_logic_ops() |> parse_first_stage() |> parse_second_stage()
  ...
  defp parse_first_stage([{:method_call, a, b, args}], ctx) do
    {:method_call, parse_first_stage([a], ctx), parse_first_stage([b], ctx), parse_first_stage_list(args, ctx)}
  end
  ...
  defp parse_second_stage({:ternary, cond, true_expr, false_expr} = op, ctx) do
    parsed = {:ternary, parse_second_stage(cond, ctx), parse_second_stage(true_expr, ctx), parse_second_stage(false_expr, ctx)}
    if parsed != op, do: parse_second_stage(parsed, ctx), else: op
  end

  defp parse_second_stage({:logical_or, a, b}, _ctx) when is_boolean(a) and is_boolean(b), do: a || b
  defp parse_second_stage({:logical_and, a, b}, _ctx) when is_boolean(a) and is_boolean(b), do: a && b
  ...
  defp parse_second_stage({:greater_than, a, b}, _ctx) when is_number(a) and is_number(b), do: a > b
  defp parse_second_stage({:less_than, a, b}, _ctx) when is_number(a) and is_number(b), do: a < b
  defp parse_second_stage({:gt_or_equals, a, b}, _ctx) when is_number(a) and is_number(b), do: a >= b
  defp parse_second_stage({:lt_or_equals, a, b}, _ctx) when is_number(a) and is_number(b), do: a <= b
  ...
  defp parse_second_stage({:not_equals, a, b}, _ctx) when is_boolean(a) and is_boolean(b), do: a != b
  defp parse_second_stage({:not_equals, a, b}, _ctx) when is_number(a) and is_number(b), do: a != b
  ...
  defp parse_second_stage({comparison_op, a, b} = op, ctx) when comparison_op in @comparison_ops do
    parsed = {comparison_op, parse_second_stage(a, ctx), parse_second_stage(b, ctx)}
    if parsed != op, do: parse_second_stage(parsed), else: op
  end


  defp parse_second_stage({:bitwise_left, a, b}, _ctx) when is_number(a) and is_number(b), do: Bitwise.<<<(a, b)
  defp parse_second_stage({:bitwise_right, a, b}, _ctx) when is_number(a) and is_number(b), do: Bitwise.>>>(a, b)
  defp parse_second_stage({:bitwise_and, a, b}, _ctx) when is_number(a) and is_number(b), do: Bitwise.band(a, b)
  defp parse_second_stage({:bitwise_or, a, b}, _ctx) when is_number(a) and is_number(b), do: Bitwise.bor(a, b)
  defp parse_second_stage({:bitwise_xor, a, b}, _ctx) when is_number(a) and is_number(b), do: Bitwise.bxor(a, b)
  ...
  defp parse_second_stage({:divide, a, b}, _ctx) when is_integer(a) and is_integer(b), do: div(a, b)
  ...
  defp parse_object_refs(lexed) do
    ...
    |> parse_object_method_calls()
  end

  defp parse_object_properties(lexed, seen \\ [])
  defp parse_object_properties([a, :dot, b | rest], seen), do: parse_object_properties([{:property, a, b} | rest], seen)
  defp parse_object_properties([{:parens, exprs} | rest], seen), do: parse_object_properties(rest, seen ++ [{:parens, parse_object_properties(exprs, [])}])
  defp parse_object_properties([{:brackets, exprs} | rest], seen), do: parse_object_properties(rest, seen ++ [{:brackets, parse_object_properties(exprs, [])}])
  defp parse_object_properties([word | rest], seen), do: parse_object_properties(rest, seen ++ [word])
  defp parse_object_properties([], seen), do: seen

  defp parse_object_method_calls(lexed, seen \\ [])
  defp parse_object_method_calls([{:property, obj, method_name}, {:parens, args} | rest], seen), do: parse_object_method_calls([{:method_call, obj, method_name, args} | rest], seen)
  defp parse_object_method_calls([{:parens, exprs} | rest], seen), do: parse_object_method_calls(rest, seen ++ [{:parens, parse_object_method_calls(exprs, [])}])
  defp parse_object_method_calls([{:brackets, exprs} | rest], seen), do: parse_object_method_calls(rest, seen ++ [{:brackets, parse_object_method_calls(exprs, [])}])
  defp parse_object_method_calls([word | rest], seen), do: parse_object_method_calls(rest, seen ++ [word])
  defp parse_object_method_calls([], seen), do: seen
  ...
  defp parse_negative_modifier([word | rest], seen), do: parse_negative_modifier(rest, seen ++ [word])
  ...
  defp parse_logic_not([:not, val | rest], seen), do: parse_logic_not([{:logical_not, val} | rest], seen)
  defp parse_logic_not([{:parens, exprs} | rest], seen), do: parse_logic_not(rest, seen ++ [{:parens, parse_logic_not(exprs, [])}])
  defp parse_logic_not([{:brackets, exprs} | rest], seen), do: parse_logic_not(rest, seen ++ [{:brackets, parse_logic_not(exprs, [])}])
  ...
  defp parse_logic_and([a, :and, b | rest], seen), do: parse_logic_and([{:logical_and, a, b} | rest], seen)
  defp parse_logic_and([{:parens, exprs} | rest], seen), do: parse_logic_and(rest, seen ++ [{:parens, parse_logic_and(exprs, [])}])
  defp parse_logic_and([{:brackets, exprs} | rest], seen), do: parse_logic_and(rest, seen ++ [{:brackets, parse_logic_and(exprs, [])}])
  ...
  defp parse_logic_or([a, :or, b | rest], seen), do: parse_logic_or([{:logical_or, a, b} | rest], seen)
  defp parse_logic_or([{:parens, exprs} | rest], seen), do: parse_logic_or(rest, seen ++ [{:parens, parse_logic_or(exprs, [])}])
  defp parse_logic_or([{:brackets, exprs} | rest], seen), do: parse_logic_or(rest, seen ++ [{:brackets, parse_logic_or(exprs, [])}])
