defmodule KaitaiToolkitTest.Formats.DynamicSizesTest do
  ...
  test "can read a full-stream byte-array" do
    ...
    io =
      binary_stream(
        <<24::unsigned-integer-8, -5::signed-integer-32, 32::signed-integer-32,
          104::signed-integer-32>>
      )

    packet = FullStreamByteArray.read!(io)
    assert <<24::unsigned-integer-8>> = packet.a_thing

    assert <<-5::signed-integer-32, 32::signed-integer-32, 104::signed-integer-32>> =
             packet.the_list
  end
